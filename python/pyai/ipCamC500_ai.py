...
# 모스키토 환경설정 변경 "C:\Program Files\mosquitto\mosquitto.conf" -> 관리자 권한으로 메모장 실행
# (아래 내용 작성 - 234줄 쯤 listenr)
# # MQTT 기본 리스너 설정
# listener 1883
# protocol mqtt
#
...
# 방화벽 설정 추가 -> 실행 -> wf.msc -> 1883, 9001 포트 열기
...
# 만약 안된다면 services.msc에 가서 서비스 재실행
...
# https://deep-learning-study.tistory.com/107 (VideoCapture 참고 사이트)
...
while cap.isOpened():
    ...
    if not ret:
        break
    result_image = detect_objects(frame)    # 모델이 객체를 탐지하는 함수를 호출
