from fastapi import FastAPI # 파이썬용 개발 api
...
app = FastAPI(  # 생성자를 통해서 postman을 대신하는 문서화가 내장되어 있음
...
class LoggingMiddleware(BaseHTTPMiddleware):    # 로그를 콘솔에 출력하는 용도
    logging.basicConfig(level = logging.INFO) # 로그 출력 하기
...
async def create_item(item: Item):  # BaseModel은 데이터 모델링을 쉽게 도와주고 유효성 검사에도 수행
